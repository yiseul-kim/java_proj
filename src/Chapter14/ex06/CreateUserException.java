...
//		super();	//생략
	...
	MyException(String message) {
		super(message);			// Exception(super클래스) 부모의 생성자에 넣어줌.
	}
}
...
class A {
	...
	MyRTException mre2 = new MyRTException("예외 메세지 : MyRTException");	// 오류 메세지를 생성자에게 넣어서 처리하는 객체 생성 
	
	// 2. 예외 던지기 ( throw : 예외를 강제로 발생시킴 )
	
	// 정수를 인풋받아서 70 이하 이면 예외를 강제로 발생시킴 (예외를 직접 처리)
	void abc_1 (int num) {
		...
		/*
		 	사용자 정의 예외 : Java에서 제공되지 않는 예외를 자신이 직접 만들어서 사용하는 예외
		 			...
		 			- 실행 예외 ( unchecked Exception, Runtime Exception) : 실행시 예외 발생. 
		 				- RuntimeException 클래스를 상속 해서 사용자 정의 예외를 생성 
		 */
