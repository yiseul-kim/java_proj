	...
	public static void main(String[] args) {
		/*
		  	...
		  	일반 예외 :  반드시 예외를 처리해야 한다. 컴파일 단계에서 예외 - 이클립스에서 오류 출력 
		  			-- throws : 예외를 미루는 방법
		  			-- try catch : 자신이 직접 예외를 미루는 방법 
		 */
		
		System.out.println("문자를 콘솔에서 한자 입력 하세요. ");
		
		//1. 수동 리소스 해제 : finally 블락에서 해제
		InputStreamReader ir1 = null;		 //콘솔에서 1문자를 읽어 드리는 객체 
		
		ir1 = new InputStreamReader(System.in);		// ir1은 콘솔에서 한(1) 문자를 읽어 들일 수 있는 객체를 활성화 

		try {
			char input = (char) ir1.read();			// byte ==> char타입으로 변환해서 input(문자하나) 변수에 담는다.
