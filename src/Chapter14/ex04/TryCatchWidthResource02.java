	...
	public static void main(String[] args) {
		/*
		  	Try Catch Width Resource : try ( 객체 선언 ) { } catch (Exception e) { }
		  		...
		  		-- 일반적인 try 구문 
		  			: try {	실행 블락 (예외)} catch ( 예외 e ) { 예외를 처리할 블락 } finally { 객체 제거 }
		 	
		 		// 객체 자동 close 됨
		 		 	: try ( 리소스 선언 부분 ) {실행 블락} catch ( 예외 e ) { 예외를 처리할 블락 }  
		 		
		 		// 자동으로 객체를 제거 할려면 
		 		   	1. AutoCloseable() 인터페이스를  구현한 클래스만 자동으로 close() 할 수 있다. 
		 		   	2. try ( 객체 선언 ) { } catch ( 예외 e ) { } 
		 */

		System.out.println("콘솔에서 한 글자만 입력하세요. >>> ");
