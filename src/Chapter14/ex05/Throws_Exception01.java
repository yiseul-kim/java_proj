...
/* 예외 처리 : 
 	...
 	2. throws : 메소드 블락에 메소드를 호출하는 쪽에서 처리하도록 미루는 것. 
		일반 예외는 반드시 처리해야 되는 예외, 실행 예외는 처리해도 되고, 안해도 괜찮.		
*/
...
class A {
	...
	void bcd () {
		...
		try {
			Thread.sleep(1000);  	// 일반예외 : 1초 동안 대기
...
class B {
	...
	void bcd() throws InterruptedException{		// 예외를 전가 : throws
		...
		Thread.sleep(1000);		// InterruptedException 예외가 발생시 상위 클래스로 전가-->abc 클래스 
	}	
}
...
class C {
	void abc() {
		try {
			bcd();
		} catch (Exception e) {		// Exception 모든 예외를 전가
			e.printStackTrace();
		}
	}
